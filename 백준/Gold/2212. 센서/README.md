...
### 핵심💡
센서 사이의 길이 배열을 내림차순 해서 계산하는 게 핵심 포인트이다. 만약 집중국(K)이 2개이면 센서와 센서 사이를 한번 건너뛸 수 있다. 3개면 두 번 건너뛸 수 있다. 따라서 길이 배열을 내림차순으로 정렬한 뒤, 집중국이 건너뛸 수 있는 만큼 빼서 즉, 배열에서 K-1 위치부터 끝까지 더하면 된다. 
